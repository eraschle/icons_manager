...
from icon_manager.rules.base import (IFilterRule, IRuleValidator, Operator,
                                     Rule, RuleAttribute)
...
class FilterRuleValidator(IRuleValidator, Generic[TRule]):
    ...
    def is_valid(self) -> bool:
        return (self.filter_rule.attribute != RuleAttribute.UNKNOWN
                and self.filter_rule.operator != Operator.UNKNOWN)
...
class ChainedRuleValidator(FilterRuleValidator[ChainedRule]):
    ...
    def is_valid(self) -> bool:
        if not super().is_valid():
            return False
        return all(filter_rule.is_valid() for rule in self.rule.rules)
