...
def get_rule_type(rule_config: dict[str, Any]) -> type[IFilterRule] | None:
...
class ContainsFileRuleBuilder(ASingleRuleBuilder[ContainsFileRule]):
    ...
    def builder_for_rules(cls) -> Iterable[str]:
        return [
            Rule.CONTAINS_FILE,
            Rule.NOT_CONTAINS_FILE,
            Rule.CONTAINS_FOLDER,
            Rule.NOT_CONTAINS_FOLDER,
        ]
...
def create_single_rule(rules: Iterable[ASingleRuleBuilder], config: dict[str, Any], **kwargs) -> ISingleRule:
    ...
    raise RuntimeError("No single rule could be created")
